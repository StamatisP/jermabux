...
#if defined(NDEBUG)
# error "Jermabux cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7e72065f3b7301595b82b11044b9bacc5760315b538234ae4b5d17017f59d7b6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Jermabux: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Jermabux Signed Message:\n";
...
{
    // Jermabux: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 1700 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 30000); // Jermabux: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // Jermabux: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Jermabux: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jermabux: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jermabux: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x58e998e3a7b614863ac8bfa705d6c11a141936f07b5f6fc8666bf8ada63958d5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Jerma985 is not as compact as he may seem. 5/26/2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 850 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043c5f31d40eb81105f4a77d925b55920dec98dda545ca5f3dc428978602ea62d146a2f0a7d8b8cb10eb99ff8e8932775ffe8c3ee761da12cca126c9cacac9d4fe") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1527373077;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084752294;

        if (fTestNet)
        {
            block.nTime    = 1527373055;
            block.nNonce   = 385518709;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xee55b46064e534dcaeba075579b94b48393862632911f050ab9e72629f5ff1c9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xb3, 0xd2 }; // Jermabux: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JermabuxMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JermabuxMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JermabuxMiner(CWallet *pwallet)
{
    printf("JermabuxMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jermabux-miner");
    ...
    try { loop {
        ...
        printf("Running JermabuxMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JermabuxMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JermabuxMiner, pwallet));
}
